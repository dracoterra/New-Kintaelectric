	...
	if ( $wp_query->max_num_pages > 1 ) :
		...
		<nav class="pagination">
			<div class="nav-previous"><?php
				...
				previous_posts_link( sprintf( esc_html__( '%s Previous', 'kintaelectric' ), sprintf( '<span class="meta-nav">%s</span>', $prev_arrow ) ) );
			?></div>
			<div class="nav-next"><?php
				/* translators: %s: HTML entity for arrow character. */
				next_posts_link( sprintf( esc_html__( 'Next %s', 'kintaelectric' ), sprintf( '<span class="meta-nav">%s</span>', $next_arrow ) ) );
			?></div>
		</nav>
	<?php endif; ?>
