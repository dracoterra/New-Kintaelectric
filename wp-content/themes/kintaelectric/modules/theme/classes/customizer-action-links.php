<?php
namespace kintaelectric\Modules\Theme\Classes;
...
class Customizer_Action_Links extends \WP_Customize_Control {
	...
	private function print_customizer_action_links() {
		...
		$button_text = __( 'Begin Setup', 'kintaelectric' );
		...
		if ( Utils::is_hello_plus_active() && ! Utils::is_hello_plus_setup_wizard_done() ) {
			...
			if ( Utils::is_elementor_active() ) {
				$button_text = __( 'Finish Setup', 'kintaelectric' );
			}
		}

		$action_link_data = [
			'image' => kintaelectric_IMAGES_URL . 'elementor.svg',
			'alt' => esc_attr__( 'Elementor', 'kintaelectric' ),
			'title' => esc_html__( 'Style your header and footer', 'kintaelectric' ),
			'message' => esc_html__( 'Customize your header and footer across your website.', 'kintaelectric' ),
			'button' => $button_text,
			'link' => $button_link,
			'underButton' => $show_text ? esc_html__( 'By clicking “Begin setup” I agree to install and activate the Hello+ plugin.', 'kintaelectric' ) : '',
		];

		$action_link_data = apply_filters( 'kintaelectric-theme/customizer/action-links', $action_link_data );
