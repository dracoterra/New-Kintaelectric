<?php
/**
 * Sistema CSS Ultra-Agresivo - WooCommerce Venezuela Pro
 * Fuerza la aplicación de CSS de manera más agresiva
 * 
...
class WVP_Ultra_Aggressive_CSS {
    ...
    private function __construct() {
        // NO aplicar CSS en init para evitar headers already sent
        // Solo registrar hooks para aplicar CSS más tarde
        
        // Aplicar CSS en wp_head con máxima prioridad
        add_action('wp_head', array($this, 'inject_css_head'), 1);
        
        // Aplicar CSS en wp_footer
        add_action('wp_footer', array($this, 'inject_css_footer'), 1);
        
        // Aplicar CSS en wp_enqueue_scripts
        add_action('wp_enqueue_scripts', array($this, 'enqueue_aggressive_css'), 999);
        
        // Aplicar CSS en template_redirect (solo en frontend)
        add_action('template_redirect', array($this, 'inject_css_template'), 1);
    }
    ...
    /**
     * Encolar CSS agresivo
     */
    public function enqueue_aggressive_css() {
        $css = $this->generate_ultra_aggressive_css();
        
        // Registrar estilo
        wp_register_style('wvp-ultra-aggressive', false);
        wp_enqueue_style('wvp-ultra-aggressive');
        
        // Agregar CSS inline
        wp_add_inline_style('wvp-ultra-aggressive', $css);
    }
    
    /**
     * Inyectar CSS en template (solo en frontend)
     */
    public function inject_css_template() {
     ...
     */
    private function inject_css_directly() {
        $css = $this->generate_ultra_aggressive_css();
        echo '<style id="wvp-ultra-aggressive-css" type="text/css">' . $css . '</style>';
    }
    
    /**
     * Generar CSS ultra-agresivo
     */
    private function generate_ultra_aggressive_css() {
        $settings = $this->get_appearance_settings();
        $primary_rgb = $this->hex_to_rgb($settings['primary_color']);
        
        $css = '
        /* CSS ULTRA-AGRESIVO - WooCommerce Venezuela Pro */
        
        /* Variables CSS con máxima prioridad */
        html:root {
            --wvp-primary-color: ' . $settings['primary_color'] . ' !important;
            --wvp-secondary-color: ' . $settings['secondary_color'] . ' !important;
            --wvp-primary-rgb: ' . ($primary_rgb ? $primary_rgb['r'] . ', ' . $primary_rgb['g'] . ', ' . $primary_rgb['b'] : '0, 124, 186') . ' !important;
        }
        
        /* RESET COMPLETO DE ESTILOS WVP */
        ...
        html body .wvp-currency-switcher,
...
// Inicializar el sistema CSS ultra-agresivo
WVP_Ultra_Aggressive_CSS::get_instance();
?>
