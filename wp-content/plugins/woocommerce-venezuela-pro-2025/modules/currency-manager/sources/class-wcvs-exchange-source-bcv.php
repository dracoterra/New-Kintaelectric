...
/**
 * WooCommerce Venezuela Suite 2025 - BCV Exchange Source
 *
 * Fuente de tipo de cambio del Banco Central de Venezuela
 *
...
class WCVS_Exchange_Source_BCV {
	 ...
	 * @var string
	 */
	const SOURCE_NAME = 'Banco Central de Venezuela';
	 ...
	 * @var string
	 */
	const API_ENDPOINT = 'https://www.bcv.org.ve/api/exchange';
	...
	public static function get_exchange_rate() {
		try {
			...
			if ( ! $data || ! isset( $data['usd'] ) ) {
				throw new Exception( 'Invalid response format from BCV' );
			}

			$rate = floatval( $data['usd'] );
			
			if ( $rate <= 0 ) {
				throw new Exception( 'Invalid exchange rate from BCV' );
			}

			return $rate;

		} catch ( Exception $e ) {
			error_log( 'BCV Exchange Rate Error: ' . $e->getMessage() );
			return false;
		}
	}
	...
	public static function get_exchange_rate_with_fallback() {
		...
		$cached_rate = get_option( 'wcvs_bcv_cached_rate', false );
		if ( $cached_rate ) {
	...
	private static function get_exchange_rate_alternative() {
		try {
			$response = wp_remote_get( 'https://www.bcv.org.ve/api/exchange/alternative', array(
				'timeout' => 30,
				'headers' => array(
					'User-Agent' => 'WooCommerce Venezuela Suite 2025'
				)
			...
			if ( ! $data || ! isset( $data['rate'] ) ) {
				throw new Exception( 'Invalid response format from BCV alternative endpoint' );
			}

			$rate = floatval( $data['rate'] );
			
			if ( $rate <= 0 ) {
				throw new Exception( 'Invalid exchange rate from BCV alternative endpoint' );
			}

			return $rate;

		} catch ( Exception $e ) {
			error_log( 'BCV Alternative Exchange Rate Error: ' . $e->getMessage() );
			return false;
		}
	}
	...
	public static function cache_exchange_rate( $rate ) {
		update_option( 'wcvs_bcv_cached_rate', $rate );
		update_option( 'wcvs_bcv_cached_rate_timestamp', current_time( 'mysql' ) );
	}
	...
	public static function get_cached_exchange_rate() {
		$cached_rate = get_option( 'wcvs_bcv_cached_rate', false );
		$cached_timestamp = get_option( 'wcvs_bcv_cached_rate_timestamp', false );
	...
	public static function validate_exchange_rate( $rate ) {
		// BCV rates should be reasonable (between 1000 and 100000 VES per USD)
